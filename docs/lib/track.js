...
(function (global) {
  ...
  function render(t) {
    ...
    if (mv && mv.significant) {
      html += '<b class="ok">검증된 것 — 움직임의 크기.</b> 최고 등급(◎)은 같은 시각·같은 조건의 ' +
        '평균보다 <b>' + mv.ratio.toFixed(2) + '배</b> 크게 움직였습니다 ' +
        '(4시간 기준, t=' + mv.t.toFixed(1) + '). 등급이 낮아질수록 이 배수도 함께 낮아지고, ' +
        '「방향 불분명(※)」 등급은 정확히 평균 수준입니다.';
    }
    if (t.verdict !== 'edge') {
